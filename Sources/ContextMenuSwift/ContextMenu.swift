...
public struct ContextMenuConstants {
    ...
    public var HorizontalMarginSpace: CGFloat = 50
...
//            // this does the trick
//            let customView = UIView(frame: CGRect(x: 0, y: 0, width: 313, height: 600))
...
//    }
        ...
        if leftClippedSpace > 0, rightClippedSpace > 0 {
            ...
            if diffY > 0 {
                if (tvX + tvW/2) > mainViewRect.width/2 { //right
                    tvX += leftClippedSpace
                    mX = tvX - MenuConstants.MenuMarginSpace - mW
                } else { //left
                    tvX -= rightClippedSpace
    ...
    func updateTargetedImageViewRect() {
        ...
        mX = tvX//MenuConstants.HorizontalMarginSpace
//        if mainViewRect.width - (tvX + tvW) < 50 {
//            mX = UIScreen.main.bounds.width - (UIScreen.main.bounds.width - (tvX + tvW)) - mW
//        }
...
//        }
    ...
    func updateTargetedImageViewPosition(animated: Bool = true) {
        ...
        if animated {
            UIView.animate(withDuration: 0.2, delay: 0, usingSpringWithDamping: 0.9, initialSpringVelocity: 6, options: [.layoutSubviews, .preferredFramesPerSecond60, .allowUserInteraction], animations: { [weak self] in
...
extension ContextMenu: UITableViewDataSource, UITableViewDelegate {
    ...
    open func tableView(_ tableView: UITableView, didDeselectRowAt indexPath: IndexPath) {
        if let cell = tableView.cellForRow(at: indexPath) as? ContextMenuCell {
            self.delegate?.contextMenuDidDeselect(self, cell: cell, targetedView: self.viewTargeted, didSelect: self.items[indexPath.row], forRowAt: indexPath.row)
        }
